...
const FEATURE_INFO = {
    ...
    'AveRooms': { default: 5.5, step: 0.01, desc: 'Average number of rooms per house' },
...
async function loadAndTrain() {
    try {
        ...
        const response = await fetch('https://raw.githubusercontent.com/ageron/handson-ml2/master/datasets/housing/housing.csv');
        ...
        // Split data
        const { X_train, X_test, y_train, y_test } = trainTestSplit(data, targets, 0.2);
...
function displayResults() {
    ...
    statsGrid.innerHTML = `
        <div class="stat-card">
            ...
            <div class="stat-desc">Model explains ${(modelStats.r2_score * 100).toFixed(2)}% of variance</div>
        </div>
        <div class="stat-card">
    ...
    formGrid.innerHTML = FEATURE_NAMES.map(name => {
        ...
        return `
            <div class="form-group">
                <label for="${name}">${name.replace(/([A-Z])/g, ' $1').trim()}</label>
